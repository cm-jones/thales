...
static void BM_BlackScholesCallPrice(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::call_price(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesPutPrice(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::put_price(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesCallDelta(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::call_delta(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesPutDelta(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::put_delta(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesCallTheta(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::call_theta(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesPutTheta(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::put_theta(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesCallRho(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::call_rho(S, K, r, sigma, T));
    }
}
...
static void BM_BlackScholesCallImpliedVolatility(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::call_implied_volatility(price, S, K, r, T));
    }
}
...
static void BM_BlackScholesPutImpliedVolatility(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(BlackScholesModel::put_implied_volatility(price, S, K, r, T));
    }
}
...
static void BM_BlackScholesNormalCDF(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        benchmark::DoNotOptimize(BlackScholesModel::call_delta(100.0, 100.0, 0.0, 0.2, 1.0));
    }
}
...
static void BM_BlackScholesNormalPDF(benchmark::State& state) {
    ...
    for (auto _ : state) {
        // We need to access the private method using a workaround
        // This is a bit hacky but necessary for benchmarking private methods
        benchmark::DoNotOptimize(BlackScholesModel::gamma(100.0, 100.0, 0.0, 0.2, 1.0));
    }
}
...
BENCHMARK(BM_BlackScholesCallPrice_Volatility)->Arg(10)->Arg(20)->Arg(30)->Arg(50);
...
// Test with different expiration times: 7 days, 30 days, 90 days, 180 days, 365 days
BENCHMARK(BM_BlackScholesCallPrice_TimeToExpiration)->Arg(7)->Arg(30)->Arg(90)->Arg(180)->Arg(365);
