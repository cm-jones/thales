...
static void BM_BlackScholesCallPrice_Scalar(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int i = 0; i < num_options; ++i) {
            results[i] = BlackScholesModel::call_price(S[i], K[i], r[i], sigma[i], T[i]);
        }
...
static void BM_BlackScholesPutPrice_Scalar(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int i = 0; i < num_options; ++i) {
            results[i] = BlackScholesModel::put_price(S[i], K[i], r[i], sigma[i], T[i]);
        }
        benchmark::DoNotOptimize(results);
        benchmark::ClobberMemory();
    }
...
static void BM_BlackScholesCallDelta_Scalar(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int i = 0; i < num_options; ++i) {
            results[i] = BlackScholesModel::call_delta(S[i], K[i], r[i], sigma[i], T[i]);
        }
        benchmark::DoNotOptimize(results);
        benchmark::ClobberMemory();
    }
...
static void BM_BlackScholesGamma_Scalar(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int i = 0; i < num_options; ++i) {
            results[i] = BlackScholesModel::gamma(S[i], K[i], r[i], sigma[i], T[i]);
        }
        benchmark::DoNotOptimize(results);
        benchmark::ClobberMemory();
    }
    ...
    if (BlackScholesVectorized::cpu_supports_avx()) {
        ...
        for (int i = 0; i < batch_size; ++i) {
            if (std::abs(scalar_results[i] - vector_results[i]) > epsilon) {
                state.SkipWithError("Validation failed: scalar and vector results differ");
                break;
            }
        }
    }
