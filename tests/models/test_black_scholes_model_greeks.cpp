...
TEST_F(BlackScholesModelGreeksTest, DeltaTest) {
    ...
    EXPECT_TRUE(isClose(call_delta, 0.63, tolerance));
    
    // Test put delta (ATM)
    double put_delta = BlackScholesModel::put_delta(S, K, r, sigma, T);
    EXPECT_TRUE(isClose(put_delta, -0.37, tolerance));
    
    // Test call delta (ITM)
    call_delta = BlackScholesModel::call_delta(110.0, K, r, sigma, T);
    EXPECT_TRUE(isClose(call_delta, 0.73, tolerance));
    
    // Test put delta (ITM)
    put_delta = BlackScholesModel::put_delta(90.0, K, r, sigma, T);
    EXPECT_TRUE(isClose(put_delta, -0.73, tolerance));
    
    // Test call delta (OTM)
    call_delta = BlackScholesModel::call_delta(90.0, K, r, sigma, T);
    EXPECT_TRUE(isClose(call_delta, 0.27, tolerance));
    
    // Test put delta (OTM)
    put_delta = BlackScholesModel::put_delta(110.0, K, r, sigma, T);
    EXPECT_TRUE(isClose(put_delta, -0.27, tolerance));
    
    // Test put-call delta parity
    EXPECT_TRUE(isClose(BlackScholesModel::call_delta(S, K, r, sigma, T) - 
...
TEST_F(BlackScholesModelGreeksTest, GammaTest) {
    ...
    EXPECT_TRUE(isClose(gamma, 0.019, tolerance));
    ...
    EXPECT_TRUE(isClose(gamma_itm, gamma_otm, tolerance));
}
...
TEST_F(BlackScholesModelGreeksTest, VegaTest) {
    ...
    EXPECT_TRUE(isClose(vega, 0.39, tolerance));
    ...
    EXPECT_TRUE(isClose(vega_itm, vega_otm, tolerance));
}

TEST_F(BlackScholesModelGreeksTest, ThetaTest) {
    // Test call theta
    double call_theta = BlackScholesModel::call_theta(S, K, r, sigma, T);
    EXPECT_TRUE(isClose(call_theta, -0.05, tolerance));
    
    // Test put theta
    double put_theta = BlackScholesModel::put_theta(S, K, r, sigma, T);
    EXPECT_TRUE(isClose(put_theta, -0.03, tolerance));
    
    // Test call theta with different time to expiration
    double call_theta_short_time = BlackScholesModel::call_theta(S, K, r, sigma, 0.25);
    double call_theta_long_time = BlackScholesModel::call_theta(S, K, r, sigma, 2.0);
    ...
    double put_theta_short_time = BlackScholesModel::put_theta(S, K, r, sigma, 0.25);
    double put_theta_long_time = BlackScholesModel::put_theta(S, K, r, sigma, 2.0);
...
TEST_F(BlackScholesModelGreeksTest, RhoTest) {
    ...
    EXPECT_TRUE(isClose(call_rho, 0.50, tolerance));
    
    // Test put rho
    double put_rho = BlackScholesModel::put_rho(S, K, r, sigma, T);
    EXPECT_TRUE(isClose(put_rho, -0.44, tolerance));
    
    // Test call rho with different time to expiration
    double call_rho_short_time = BlackScholesModel::call_rho(S, K, r, sigma, 0.25);
    double call_rho_long_time = BlackScholesModel::call_rho(S, K, r, sigma, 2.0);
    ...
    double put_rho_short_time = BlackScholesModel::put_rho(S, K, r, sigma, 0.25);
...
TEST_F(BlackScholesModelGreeksTest, GreeksAtExpirationTest) {
    ...
    EXPECT_TRUE(isClose(call_delta, 0.5, 0.1));
    EXPECT_TRUE(isClose(put_delta, -0.5, 0.1));
    ...
    EXPECT_TRUE(isClose(vega, 0.0, 0.1));
    ...
    EXPECT_TRUE(isClose(call_rho, 0.0, 0.1));
    EXPECT_TRUE(isClose(put_rho, 0.0, 0.1));
}

TEST_F(BlackScholesModelGreeksTest, GreeksDeepITMOTMTest) {
    // Deep ITM call (S >> K)
    double S_itm = 150.0;
    double call_delta_itm = BlackScholesModel::call_delta(S_itm, K, r, sigma, T);
    ...
    EXPECT_TRUE(isClose(call_delta_itm, 1.0, 0.05));
    EXPECT_TRUE(isClose(put_delta_itm, 0.0, 0.05));
    
    // Deep OTM call (S << K)
    double S_otm = 50.0;
    double call_delta_otm = BlackScholesModel::call_delta(S_otm, K, r, sigma, T);
    ...
    EXPECT_TRUE(isClose(call_delta_otm, 0.0, 0.05));
    EXPECT_TRUE(isClose(put_delta_otm, -1.0, 0.05));
