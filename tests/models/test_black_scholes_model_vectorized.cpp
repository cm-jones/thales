...
void generate_test_data(int size, 
                      std::vector<double>& S, 
                      std::vector<double>& K, 
                      std::vector<double>& r, 
    ...
    std::uniform_real_distribution<> r_dist(0.01, 0.05);       // Risk-free rate (1-5%)
    std::uniform_real_distribution<> sigma_dist(0.1, 0.5);     // Volatility (10-50%)
    std::uniform_real_distribution<> T_dist(0.1, 2.0);         // Time to expiry (0.1-2 years)
...
TEST_F(BlackScholesVectorizedTest, TestCallPrice) {
    ...
    for (int i = 0; i < num_options; ++i) {
        scalar_results[i] = BlackScholesModel::call_price(S[i], K[i], r[i], sigma[i], T[i]);
    }
...
TEST_F(BlackScholesVectorizedTest, TestPutPrice) {
    ...
    std::vector<double> scalar_results(num_options);
    for (int i = 0; i < num_options; ++i) {
        scalar_results[i] = BlackScholesModel::put_price(S[i], K[i], r[i], sigma[i], T[i]);
    }
...
TEST_F(BlackScholesVectorizedTest, TestCallDelta) {
    ...
    std::vector<double> scalar_results(num_options);
    for (int i = 0; i < num_options; ++i) {
        scalar_results[i] = BlackScholesModel::call_delta(S[i], K[i], r[i], sigma[i], T[i]);
    }
...
TEST_F(BlackScholesVectorizedTest, TestGamma) {
    ...
    std::vector<double> scalar_results(num_options);
    for (int i = 0; i < num_options; ++i) {
        scalar_results[i] = BlackScholesModel::gamma(S[i], K[i], r[i], sigma[i], T[i]);
    }
...
TEST_F(BlackScholesVectorizedTest, TestExpiredOptions) {
    ...
    for (int i = 0; i < num_options; ++i) {
        scalar_call_results[i] = BlackScholesModel::call_price(S[i], K[i], r[i], sigma[i], T[i]);
        scalar_put_results[i] = BlackScholesModel::put_price(S[i], K[i], r[i], sigma[i], T[i]);
    }
...
TEST_F(BlackScholesVectorizedTest, TestInvalidInputs) {
    ...
    for (int i = 1; i < num_options; ++i) {
        ...
        EXPECT_EQ(scalar_is_nan, vector_is_nan)
            << "At index " << i << ": Scalar " << (scalar_is_nan ? "is" : "is not")
            << " NaN but Vector " << (vector_is_nan ? "is" : "is not") << " NaN";
    }
}
