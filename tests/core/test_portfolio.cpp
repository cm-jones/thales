...
TEST_F(PortfolioTest, UpdatePosition) {
    ...
    EXPECT_EQ(updated_position.unrealized_pnl, updated_position.get_unrealized_pnl());
    EXPECT_EQ(updated_position.get_unrealized_pnl(), 2000.0); // (170 - 150) * 100
}
...
TEST_F(PortfolioTest, AddOrder) {
    ...
    // Create a test order
    auto symbol_id = SymbolLookup::get_instance().get_id("AAPL");
    Order order("order1", symbol_id, Order::Type::LIMIT, Order::Side::BUY, 100, 150.0);
...
TEST_F(PortfolioTest, UpdateOrder) {
    ...
    portfolio.update_order("order1", Order::Status::PARTIALLY_FILLED, 50, 149.0);
    ...
    EXPECT_EQ(symbol_orders[0].average_fill_price, 150.0); // (149*50 + 151*50)/100
}
...
TEST_F(PortfolioTest, CancelOrder) {
    ...
    // But it should still be in the orders list for the symbol with CANCELED status
    auto symbol_orders = portfolio.get_orders("AAPL");
    EXPECT_EQ(symbol_orders.size(), 1);
...
TEST_F(PortfolioTest, MultiplePositionsAndOrders) {
    ...
    // Add multiple orders
    Order aapl_order("order1", aapl_id, Order::Type::LIMIT, Order::Side::BUY, 50, 155.0);
    Order msft_order("order2", msft_id, Order::Type::MARKET, Order::Side::SELL, 25);
    Order goog_order("order3", goog_id, Order::Type::STOP, Order::Side::SELL, 10, 0.0, 2050.0);
    ...
    double expected_value = 
        aapl_position.get_value() + 
        msft_position.get_value() + 
        goog_position.get_value();
    
    double expected_unrealized_pnl = 
        aapl_position.unrealized_pnl + 
