...
TEST_F(ContractTest, DefaultConstructor) {
    ...
    EXPECT_EQ(std::string(contract.exchange.data()), "");
    EXPECT_EQ(contract.type, Option::Type::UNKNOWN);
    EXPECT_EQ(std::string(contract.expiry.data()), "");
...
TEST_F(ContractTest, ParameterizedConstructor) {
    ...
    EXPECT_EQ(std::string(contract.exchange.data()), exchange);
    EXPECT_EQ(contract.type, type);
    EXPECT_EQ(std::string(contract.expiry.data()), expiry);
...
TEST_F(ContractTest, ContractWithDifferentOptionTypes) {
    ...
    Option call_contract(symbol_id, exchange, Option::Type::CALL, expiry,
                         strike);
