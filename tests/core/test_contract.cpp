...
TEST_F(ContractTest, DefaultConstructor) {
    ...
    EXPECT_EQ(contract.type, Contract::Type::UNKNOWN);
...
TEST_F(ContractTest, ParameterizedConstructor) {
    ...
    Contract::Type type = Contract::Type::CALL;
    std::string expiry = "2025-12-19";
    double strike = 200.0;
    
    Contract contract(symbol_id, exchange, type, expiry, strike);
...
TEST_F(ContractTest, ContractWithDifferentOptionTypes) {
    ...
    Contract call_contract(symbol_id, exchange, Contract::Type::CALL, expiry, strike);
    EXPECT_EQ(call_contract.type, Contract::Type::CALL);
    
    // Test with PUT option
    Contract put_contract(symbol_id, exchange, Contract::Type::PUT, expiry, strike);
    EXPECT_EQ(put_contract.type, Contract::Type::PUT);
}
...
TEST_F(ContractTest, ContractWithDifferentExpiryAndStrike) {
    ...
    Contract contract1(symbol_id, exchange, type, "2025-06-20", 180.0);
    EXPECT_EQ(contract1.expiry, "2025-06-20");
    EXPECT_EQ(contract1.strike, 180.0);
    
    Contract contract2(symbol_id, exchange, type, "2025-09-19", 190.0);
    EXPECT_EQ(contract2.expiry, "2025-09-19");
    EXPECT_EQ(contract2.strike, 190.0);
    
    Contract contract3(symbol_id, exchange, type, "2025-12-19", 200.0);
