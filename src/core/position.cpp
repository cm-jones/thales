...
Position::Position(utils::SymbolLookup::SymbolID id,
                   const std::string &exchange, Option::Type option_type,
                   int qty, double avg_price, double curr_price,
                   double unrealized_pnl, double realized_pnl)
    : Position(PositionParams{.id = id,
                              .exchange = exchange,
...
double Position::get_value() const {
    return static_cast<double>(quantity) * last_price;
}
