...
bool DbLogger::initialize(
    ...
    instance_->worker_thread_ = std::thread(&DbLogger::worker_thread, instance_.get());
...
bool DbLogger::connect() {
    ...
    try {
        ...
        try {
            ...
            logger.warning("Could not connect to database: " + std::string(e.what()));
            ...
            try {
                ...
                std::string checkDbQuery = "SELECT 1 FROM pg_database WHERE datname = " + txn.quote(db_name_);
                pqxx::result result = txn.exec(checkDbQuery);
                
                if (result.empty()) {
                    // Database doesn't exist, create it
                    std::string createDbQuery = "CREATE DATABASE " + txn.esc(db_name_);
                    txn.exec(createDbQuery);
                    txn.commit();
                    
                    logger.info("Database '" + db_name_ + "' created successfully");
                    
                    // Now try to connect to the new database
                    auto conn = std::make_unique<pqxx::connection>(connectionString);
                    
                    if (conn->is_open()) {
                        logger.info("Connected to newly created PostgreSQL database: " + db_name_);
                        connected_ = true;
                    ...
                    // Database exists but we couldn't connect, might be a permission issue
                    logger.error("Database '" + db_name_ + "' exists but connection failed. Check permissions.");
                }
            } catch (const std::exception& e) {
                logger.error("Failed to create database: " + std::string(e.what()));
                logger.info("You may need to run the setup script with superuser privileges:");
        ...
        logger.error("Exception while connecting to database: " + std::string(e.what()));
        connected_ = false;
        return false;
    }
...
bool DbLogger::create_tables_if_not_exist() {
    ...
    try {
        ...
        txn.exec("CREATE INDEX IF NOT EXISTS idx_trade_executions_timestamp ON trade_executions(timestamp)");
        txn.exec("CREATE INDEX IF NOT EXISTS idx_trade_executions_symbol ON trade_executions(symbol)");
        txn.exec("CREATE INDEX IF NOT EXISTS idx_trade_executions_strategy ON trade_executions(strategy_name)");
        ...
        logger.error("Exception while creating database tables: " + std::string(e.what()));
        return false;
    }
}
...
void DbLogger::worker_thread() {
    ...
    while (running_) {
        try {
            ...
            logger.error("Exception in database logger worker thread: " + std::string(e.what()));
        }
    }
    ...
    try {
        ...
        logger.error("Exception while processing remaining logs: " + std::string(e.what()));
    }
...
bool DbLogger::insert_log_batch(const std::vector<TradeExecutionLog>& logs) {
    ...
    try {
        ...
        for (const auto& log : logs) {
            txn.exec_prepared("insert_trade_execution",
                log.timestamp,
                log.strategy_name,
                log.symbol,
                log.order_id,
        ...
        logger.error("Exception while inserting log batch: " + std::string(e.what()));
        connected_ = false;
        return false;
    }
}
...
bool DbLogger::is_connected() const {
    return connected_;
}
...
size_t DbLogger::get_failed_count() const {
