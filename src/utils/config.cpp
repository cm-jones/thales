        ...
        while (std::getline(file, line)) {
            ...
            if (in_array) {
                ...
                if (line.find('-') == std::string::npos && 
                    (line.find(':') != std::string::npos || trimmed_line.empty())) {
                    // Process the array
                    std::string full_key = current_path.empty() ? array_key : current_path + "." + array_key;
            ...
            if (colon_pos != std::string::npos) {
                ...
                if ((value.empty() || value.size() >= 2 && value.front() == '[' && value.back() == ']') &&
                    std::getline(file, line) && line.find('-') != std::string::npos) {
                ...
                std::string full_key = current_path.empty() ? key : current_path + "." + key;
                ...
                } else if (!value.empty() && value.find('.') != std::string::npos) {
...
bool Config::save_to_file(const std::string& file_path) const {
    ...
    try {
        ...
        std::unordered_map<std::string, std::vector<std::pair<std::string, ConfigValue>>> sections;
...
void Config::writeYamlValue(std::ofstream& file, const std::string& key, 
    ...
    std::visit([&file, &indent_str](const auto& v) {
        ...
        if constexpr (std::is_same_v<T, bool>) {
            ...
            for (const auto& item : v) {
                ...
                } else if constexpr (std::is_same_v<std::decay_t<decltype(item)>, std::string>) {
                    // Add quotes if needed
                    if (item.find(':') != std::string::npos || item.find('#') != std::string::npos ||
                        item.empty() || std::isspace(item.front()) || std::isspace(item.back())) {
                        file << "\"" << item << "\"" << "\n";
                    } else {
                        file << item << "\n";
                    }
