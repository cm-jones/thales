...
bool BlackScholes::initialize() {
    ...
    try {
        ...
        if (config_.has_key("strategies.BlackScholes.minPriceDiscrepancy")) {
            min_price_discrepancy_ = config_.get_double(
                "strategies.BlackScholes.minPriceDiscrepancy");
        }

        if (config_.has_key("strategies.BlackScholes.minVolatility")) {
            min_volatility_ =
                config_.get_double("strategies.BlackScholes.minVolatility");
        }

        if (config_.has_key("strategies.BlackScholes.maxVolatility")) {
            max_volatility_ =
                config_.get_double("strategies.BlackScholes.maxVolatility");
        }

        if (config_.has_key("strategies.BlackScholes.minDaysToExpiration")) {
            min_days_to_expiration_ =
                config_.get_int("strategies.BlackScholes.minDaysToExpiration");
        }

        if (config_.has_key("strategies.BlackScholes.maxDaysToExpiration")) {
            max_days_to_expiration_ =
                config_.get_int("strategies.BlackScholes.maxDaysToExpiration");
        }
        ...
        logger.error("Exception during BlackScholes initialization: " +
                     std::string(e.what()));
        return false;
    } catch (...) {
    ...
    try {
        ...
        for (const auto& data : market_data) {
            ...
            double time_to_expiration =
                calculate_time_to_expiration(option_data->expirationDate);

            // Calculate the theoretical price using Black-Scholes
            double theoretical_price = 0.0;
            if (option_data->isCall) {
                theoretical_price = models::BlackScholes::call_price(
                    underlying_price, option_data->strikePrice, risk_free_rate,
                    volatility, time_to_expiration);
            } else {
                theoretical_price = models::BlackScholes::put_price(
                    underlying_price, option_data->strikePrice, risk_free_rate,
                    volatility, time_to_expiration);
            }
            ...
            if (std::abs(price_discrepancy) >= min_price_discrepancy_) {
                Signal signal =
                    generate_signal(*option_data, theoretical_price);
                signals.push_back(signal);

                logger.info("Generated signal for " + option_data->symbol +
                            ": Market price = " + std::to_string(market_price) +
                            ", Theoretical price = " +
                            std::to_string(theoretical_price) +
                            ", Discrepancy = " +
                            std::to_string(price_discrepancy * 100.0) + "%");
            }
        }

        logger.debug("BlackScholes strategy generated " +
                     std::to_string(signals.size()) + " signals");
        return signals;
    } catch (const std::exception& e) {
        logger.error("Exception during BlackScholes execution: " +
                     std::string(e.what()));
        return signals;
    } catch (...) {
...
std::vector<std::string> BlackScholes::get_symbols() const { return symbols_; }
...
double BlackScholes::calculate_time_to_expiration(
...
Signal BlackScholes::generate_signal(const data::OptionData& option,
    ...
    return Signal(option.symbol, signal_type, strength, theoretical_price,
                  0.0,  // No stop loss for now
                  timestamp, "BlackScholes");
}
