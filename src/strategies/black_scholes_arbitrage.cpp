...
bool BlackScholesArbitrage::initialize() {
    ...
    if (config_.has_key("strategies.BlackScholesArbitrage.minPriceDiscrepancy")) {
    ...
    if (config_.has_key("strategies.BlackScholesArbitrage.minVolatility")) {
        min_volatility_ =
            config_.get_double("strategies.BlackScholesArbitrage.minVolatility");
    }

    if (config_.has_key("strategies.BlackScholesArbitrage.maxVolatility")) {
        max_volatility_ =
            config_.get_double("strategies.BlackScholesArbitrage.maxVolatility");
    }

    if (config_.has_key("strategies.BlackScholesArbitrage.minDaysToExpiration")) {
        min_days_to_expiration_ =
            config_.get_int("strategies.BlackScholesArbitrage.minDaysToExpiration");
    }

    if (config_.has_key("strategies.BlackScholesArbitrage.maxDaysToExpiration")) {
        max_days_to_expiration_ =
            config_.get_int("strategies.BlackScholesArbitrage.maxDaysToExpiration");
    }
    ...
    if (symbols_.empty()) {
        logger.error("Failed to load symbols for BlackScholesArbitrage strategy");
        return false;
    }
...
std::vector<std::string> BlackScholesArbitrage::get_symbols() const { return symbols_; }
...
double BlackScholesArbitrage::calculate_historical_volatility(
    ...
    double variance = sum_squared_diff / static_cast<double>(returns.size() - 1);
...
bool BlackScholesArbitrage::is_option_eligible(const data::OptionData& option) const {
    // Parse the expiration date (format: YYYY-MM-DD)
    std::tm expiration_tm = {};
...
Signal BlackScholesArbitrage::generate_signal(const data::OptionData& option,
    ...
    return Signal(option.symbol, signal_type, strength, theoretical_price,
                  ...
                  timestamp, "BlackScholesArbitrage");
}
