...
void BlackScholesVectorized::vectorized_call_price(
    ...
    if (!cpu_supports_avx()) {
        // Fallback to scalar implementation if AVX not supported
        for (int i = 0; i < n; ++i) {
    ...
    // Process 4 options at a time
    for (int i = 0; i < n - 3; i += 4) {
        // Load 4 sets of inputs
        __m256d S = _mm256_loadu_pd(&S_array[i]);
        __m256d K = _mm256_loadu_pd(&K_array[i]);
        __m256d r = _mm256_loadu_pd(&r_array[i]);
        __m256d sigma = _mm256_loadu_pd(&sigma_array[i]);
        __m256d T = _mm256_loadu_pd(&T_array[i]);
        ...
        __m256d result = _mm256_blendv_pd(option_price, intrinsic_value, expired_mask);
...
void BlackScholesVectorized::vectorized_put_price(
    const double* S_array,
    const double* K_array,
    const double* r_array,
    const double* sigma_array,
...
void BlackScholesVectorized::vectorized_call_delta(
    ...
    for (int i = 0; i < n - 3; i += 4) {
        ...
        // For expired options, delta is either 1.0 (if S > K) or 0.0 (if S <= K)
...
void BlackScholesVectorized::vectorized_gamma(
    const double* S_array,
    const double* K_array,
    const double* r_array,
    const double* sigma_array,
...
#ifdef THALES_X86_OR_X64
// Helper function to compute normal CDF for vector of values using approximation
__m256d BlackScholesVectorized::normal_cdf_vector(__m256d x) {
    ...
    __m256d cexp_t_poly = _mm256_mul_pd(c, _mm256_mul_pd(exp_val, _mm256_mul_pd(t, poly)));
    __m256d result = _mm256_blendv_pd(cexp_t_poly, _mm256_sub_pd(one, cexp_t_poly), mask);
...
__m256d BlackScholesVectorized::normal_pdf_vector(__m256d x) {
    ...
    // -x²/2
    __m256d x_squared = _mm256_mul_pd(x, x);
    __m256d neg_x_squared_half = _mm256_mul_pd(x_squared, _mm256_sub_pd(zero, half));
...
__m256d BlackScholesVectorized::d1_vector(__m256d S, __m256d K, __m256d r, __m256d sigma, __m256d T) {
    const __m256d zero = _mm256_set1_pd(0.0);
    const __m256d half = _mm256_set1_pd(0.5);
    ...
    __m256d nan_vector = _mm256_set1_pd(std::numeric_limits<double>::quiet_NaN());
    return _mm256_blendv_pd(nan_vector, d1, valid_mask);
}

// Helper function to compute d2 parameter
__m256d BlackScholesVectorized::d2_vector(__m256d S, __m256d K, __m256d r, __m256d sigma, __m256d T) {
    __m256d d1 = d1_vector(S, K, r, sigma, T);
    __m256d sqrt_T = _mm256_sqrt_pd(T);
