...
struct alignas(64) Signal {
    ...
    // Default constructor
    Signal()
        : type(Type::HOLD), symbol(""), timestamp(""), strategy_name(""),
          strength(0.0), target_price(0.0), stop_loss(0.0) {}

    // Constructor using parameter struct
    explicit Signal(const SignalParams &params)
        : type(params.type), symbol(params.symbol), timestamp(params.timestamp),
          strategy_name(params.strategy_name), strength(params.strength),
          target_price(params.target_price), stop_loss(params.stop_loss) {}
};
