...
class IBWrapper {
  public:
    ...
    // Market data methods
    void tickPrice(int tickerId, int field, double price,
    ...
    void realtimeBar(int reqId, long time, double open, double high, double low,
                     double close, long volume, double wap, int count);
    void historicalData(int reqId, const std::string &date, double open,
    ...
    void openOrder(int orderId, const IBContract &contract,
                   const IBOrder &order, const IBOrderState &orderState);
    void openOrderEnd();
    void completedOrder(const IBContract &contract, const IBOrder &order,
                        const IBOrderState &orderState);
    void completedOrdersEnd();
    void execDetails(int reqId, const IBContract &contract,
                     const IBExecution &execution);
    ...
    void position(const std::string &account, const IBContract &contract,
    ...
    void bondContractDetails(int reqId,
                             const IBContractDetails &contractDetails);
    ...
    void softDollarTiers(int reqId,
                         const std::vector<IB::SoftDollarTier> &tiers);
    void familyCodes(const std::vector<IB::FamilyCode> &familyCodes);
    // TODO: Define IBContractDescription class
    void symbolSamples(
        int reqId,
        const std::vector<IBContractDescription> &contractDescriptions);
    void mktDepthExchanges(const std::vector<IB::DepthMktDataDescription>
                               &depthMktDataDescriptions);
    void smartComponents(int reqId,
                         const std::map<int, IB::SmartComponent> &theMap);
    ...
    void marketRule(int marketRuleId,
                    const std::vector<IB::PriceIncrement> &priceIncrements);
    ...
    void tickByTickAllLast(int reqId, int tickType, long time, double price,
                           int size, const IB::TickAttribLast &tickAttribLast,
                           const std::string &exchange,
                           const std::string &specialConditions);
    void tickByTickBidAsk(int reqId, long time, double bidPrice,
                          double askPrice, int bidSize, int askSize,
                          const IB::TickAttribBidAsk &tickAttribBidAsk);
  ...
  private:
    ...
    MarketData convertToMarketData(int tickerId, double price, int size,
                                   const std::string &timestamp);
    core::Order convertToOrder(const IBOrder &ibOrder,
                               const IBContract &ibContract);
    core::Position convertToPosition(const IBContract &ibContract,
                                     double position, double marketPrice,
                                     double averageCost);
