...
struct MarketData {
    ...
    MarketData(const std::string &sym = "", const std::string &ts = "",
        ...
        : symbol(sym), timestamp(ts), price(p), open(o), high(h), low(l),
          close(c), volume(v), bid(b), ask(a), bid_size(bs), ask_size(as) {}
};
...
struct OptionData : public MarketData {
    ...
    OptionData(const std::string &sym = "", const std::string &ts = "",
        ...
        : MarketData(sym, ts, p, o, h, l, c, v, b, a, bs, as),
          underlying_symbol(underlying), expiration_date(exp_date),
          strike_price(strike), implied_volatility(iv), delta(d), gamma(g),
          theta(t), vega(vg), rho(r), open_interest(oi), is_call(call) {}
};
...
struct Bar {
    ...
    Bar(const std::string &sym = "", const std::string &ts = "",
        ...
        : symbol(sym), timestamp(ts), interval(intv), open(o), high(h), low(l),
          close(c), volume(v) {}
};
